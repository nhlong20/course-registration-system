...
public class CourseStudentDAO {
    ...
    public static List<Course> getAllCourses(String studentId) {
        ...
        try(Session session = HibernateUtil.getSessionFactory()
                .openSession()) {
            String hql = "select cs.course from CourseStudent cs where cs.student.studentId = :studentId";
            Query query = session.createQuery(hql);
    ...
    public static boolean add(CourseStudent courseStudent){
        ...
        List<Course> registedCourses = getAllCourses(curStudentId).stream()
                .filter(p -> p.getSemester().getSemesterId() == currentSemId).collect(Collectors.toList());
